...
DESIGN NOTES:
- user_id FK with ondelete="CASCADE": if a user is deleted, their addresses
  are automatically deleted by the DATABASE, not by application code.

- is_default: exactly one address per user should normally be the default
  shipping address.

  The API enforces this at the application layer:
  when an address becomes default, all other addresses belonging to the
  same user are first changed to is_default=False.

  Every address query is also scoped to the authenticated user so one
