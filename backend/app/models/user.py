...
DESIGN NOTES:
- UUID is used instead of an auto-incrementing integer for user IDs.
- email is unique and indexed because it is used for login.
- hashed_password is nullable for future OAuth users.
- role is a database-backed enum.
- is_active allows accounts to be disabled without deleting data.
- is_email_verified tracks email verification state.
- addresses and wishlist_items are owned by the user and are deleted
  automatically when the user is deleted.
"""
