...
1. mock
   - Used for local development when Razorpay credentials are not available.
   - Does not contact Razorpay.
   - Creates a local Payment record directly.
   - Allows the Admin Payments page and refund flow to be tested.

2. razorpay
   - Uses the real Razorpay integration.
   - Used when valid Razorpay credentials are available.
...
from app.core.notifications import notify_user
from app.core.rate_limit import PAYMENT_RATE_LIMIT, limiter
from app.core.razorpay_gateway import (
    RazorpayGateway,
...
def _is_mock_payment() -> bool:
    """Return True when the application is running in mock payment mode."""
    return settings.PAYMENT_MODE.lower() == "mock"
...
def _get_key_id() -> str:
    ...
    In mock mode this value is not actually used by the backend,
    but the existing response schema still expects it.
    """
    return settings.RAZORPAY_KEY_ID
...
def create_razorpay_order(
    ...
    In mock mode:
        ...
        - A local Payment record is created.
        - The payment is immediately marked as PAID.

    In Razorpay mode:
        - A real Razorpay order is created.
        - A local Payment record is created with CREATED status.
    """
    ...
    if _is_mock_payment():
        ...
        We generate IDs that clearly identify the payment as mock.
        No Razorpay network request is made.
        """

        mock_razorpay_order_id = (
            f"mock_order_{uuid.uuid4().hex[:12]}"
        )

        mock_razorpay_payment_id = (
            f"mock_payment_{uuid.uuid4().hex[:12]}"
        )

        payment = Payment(
            order_id=order.id,
            razorpay_order_id=mock_razorpay_order_id,
            razorpay_payment_id=mock_razorpay_payment_id,
            amount=order.total_amount,
            currency="INR",
        ...
        # Notify the customer that the development payment succeeded.
        await_notification = True

        # `notify_user` is async, but this endpoint is synchronous.
        # We intentionally don't call it here because the mock payment
        # itself is already represented as successful in the database.
...
async def verify_razorpay_payment(
    ...
    In mock mode, mock payment IDs are already marked as PAID,
    so no Razorpay signature verification is required.

    In real Razorpay mode, HMAC signature verification is performed.
    """
    ...
    # MOCK MODE
    # ---------------------------------------------------------
    if _is_mock_payment() and payment.razorpay_order_id.startswith(
        "mock_order_"
    ):
        payment.status = PaymentStatus.PAID
...
async def razorpay_webhook(
    ...
    Webhooks are only relevant to the real Razorpay flow.
    Mock payments do not need Razorpay webhooks.
    """

    # Mock mode doesn't receive Razorpay webhooks.
    if _is_mock_payment():
...
def get_order_payments(
    ...
    """
    Return payment history for a customer's order.
    """
...
def refund_payment(
    ...
    Mock payment:
        ...
        - Payment becomes REFUNDED locally.

    Real Razorpay payment:
        - Razorpay refund API is called.
        - Payment becomes REFUNDED after successful response.
    """
    ...
    if payment is None or payment.status != PaymentStatus.PAID:
        raise HTTPException(
            status.HTTP_404_NOT_FOUND,
            "No paid payment found with this ID",
        )
    ...
    if (
        ...
        and payment.razorpay_payment_id.startswith(
            "mock_payment_"
        )
    ):
        payment.status = PaymentStatus.REFUNDED
    ...
    gateway.create_refund(
        payment.razorpay_payment_id,
        _rupees_to_paise(refund_amount),
    )
...
def admin_list_payments(
    ...
    Works for both mock and real Razorpay payments because
    both are stored in the same Payment table.
    """
